...
def _main_():
    ...
    try:
        ...
        with open('badminton/config.txt', 'w', encoding='utf8') as w:
...
def print_win(print_list):
    ...
    if len(uses) > 1:
        current = datetime.datetime.strptime(uses[0]['date'], '%Y-%m-%d') - datetime.datetime.today()
        autoPickUpCourt.manual_exec(current.days + 1, uses[0]['site'],
                                    [str(uses[0]['time']).replace(':00', '').replace(':30', ''),
                                     str(uses[1]['time']).replace(':00', '').replace(':30', '')])
        autoPickUpCourt.prepare()
...
def send_wechat_msg(msg):
    resp = requests.get(
        'https://sc.ftqq.com/{}.send?text={}&desp={}'.format(sckey, '场地提醒', msg)
    )
...
if __name__ == '__main__':
    while True:
        try:
            ...
            traceback.print_exc()
        requests.get(
            'https://sc.ftqq.com/{}.send?text={}&desp={}'.format(sckey, '需要重启', '万体监控报错了')
        )
        time.sleep(10)
    input("回车键退出")
    sys.exit()
