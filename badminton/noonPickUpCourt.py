...
def _main_():
    global timespans, week_timespans, random_start, random_end, wechat_name, names, wechat_list, is_agent, exec_time
    try:
    ...
    clear_index = 0
    wechat_list = []
    # thread = threading.Thread(target=schedule_task, kwargs={'exec_time': '10:48'})
    # thread.start()
    ...
    try:
        with ThreadPoolExecutor(max_workers=None) as t:
            ...
            for item in group_param:
                ...
                item_date = datetime.datetime(today.year, today.month, today.day) + datetime.timedelta(days=7)
...
def schedule_task(exec_time):
    ...
    schedule.every().day.at(exec_time).do(exec_search)
    global task_is_run
    while True:
        if task_is_run:
            task_is_run = False
